...
const openReel=(url)=>{if(!reelModal||!reelFrame)return;reelFrame.src=url;reelOpenLink.href=url;reelModal.classList.add('open');reelModal.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';};
document.querySelectorAll('.reel-trigger').forEach(card=>card.addEventListener('click',()=>openReel(card.dataset.reelUrl)));
...
(function(){
  ...
  videos.forEach(video=>{
    ...
    const sound=frame?.querySelector('.reel-sound-toggle, .hero-video-sound');
  ...
  // Reel visibility autoplay. Never restart a video the user intentionally paused.
...
// v8.16 — horizontal navigation for native Instagram Reel embeds.
(function(){
  const root=document.querySelector('[data-slider="instagram-reels"]');
  const track=root?.querySelector('.instagram-reel-track');
  if(!root||!track) return;
  const cards=[...track.querySelectorAll('.instagram-reel-card')];
  const step=()=>cards[0]?.getBoundingClientRect().width + 18 || root.clientWidth*.85;
  document.querySelector('[data-slider-next="instagram-reels"]')?.addEventListener('click',()=>track.scrollBy({left:step(),behavior:'smooth'}));
  document.querySelector('[data-slider-prev="instagram-reels"]')?.addEventListener('click',()=>track.scrollBy({left:-step(),behavior:'smooth'}));
})();

// v8.20 — hero direct MP4 controls with user-gesture audio support
(function(){
  const frame=document.querySelector('.hero-video-frame');
  const video=frame?.querySelector('.hero-video');
  const play=frame?.querySelector('.hero-video-toggle');
  const sound=frame?.querySelector('.hero-video-sound');
  if(!video) return;
  ...
  const playWithAudio=()=>{
    ...
    const p=video.play();
    if(p?.catch) p.catch(()=>{
      // Some browsers may reject unmuted playback; keep the video usable.
      video.muted=true;
      video.play().catch(()=>{});
    });
  };
  ...
  play?.addEventListener('click',e=>{
    ...
    if(video.paused) playWithAudio();
    else video.pause();
  });

  sound?.addEventListener('click',e=>{
    e.preventDefault(); e.stopPropagation();
    if(video.muted) playWithAudio();
    else { video.muted=true; video.volume=1; }
    sync();
  });
  ...
  video.addEventListener('canplay',()=>{
    if(video.paused && !video.dataset.userPaused) playMuted();
  });
